...
struct HashAggStats {
    ...
    sn_keccak_inputs: HashSet<Vec<u64>>,
}
...
#[inline]
pub fn record_pedersen_call(lhs: Felt, rhs: Felt) {
    if !enabled() {
        return;
    }
    STATS.with(|stats| {
        ...
        stats.pedersen_inputs.insert((lhs, rhs));
    });
}
...
#[inline]
pub fn record_sn_keccak_call(data: &[u64], hit: bool) {
    if !enabled() {
        return;
    }
    STATS.with(|stats| {
